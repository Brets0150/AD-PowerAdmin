...
function Show-InstallHelp {
    ...
    Write-Host ""
    Write-HelpLine "This guide covers four topics:"
    ...
    Write-HelpLine "  Part 4 -- Troubleshooting (diagnosing and fixing common problems)"
    ...
    Write-Host ""
    Write-HelpLine "The installer begins with a directory confirmation prompt, then"
    Write-HelpLine "performs six numbered steps in this order:"

    # =========================================================================
    Write-HelpSection "Initial Confirmation -- Installation Directory"
    Write-HelpLine "Before any numbered step runs, the installer shows the current value"
    ...
    # =========================================================================
    Write-HelpStep "1" "Create and Harden the Install Directory"
    ...
    Write-HelpLine "This step always runs unconditionally -- re-running the installer will"
    Write-HelpLine "re-verify and re-apply the directory ACL and audit settings."
    Write-Host ""
    ...
    # =========================================================================
    Write-HelpStep "2" "Deploy Production Files"
    ...
    # =========================================================================
    Write-HelpStep "3" "Create the Standalone Managed Service Account (sMSA)"
    ...
    # =========================================================================
    Write-HelpStep "4" "Modify the Default Domain Controllers Policy GPO"
    ...
    # =========================================================================
    Write-HelpStep "5" "Register the AD-PowerAdmin_Daily Scheduled Task"
    ...
    # =========================================================================
    Write-HelpStep "6" "Grant sMSA File System Permissions"
    Write-HelpLine "Calls Set-ReportsFolderAcl to grant the sMSA explicit Access Control"
    Write-HelpLine "Entries on two paths. This step always runs, even when no files are"
    Write-HelpLine "copied, so permissions are verified and repaired on every install run."
    Write-Host ""
    Write-HelpLine "Why explicit ACEs are required for the sMSA:"
    Write-HelpDetail "When a scheduled task runs under an sMSA, Windows evaluates filesystem"
    Write-HelpDetail "permissions using the sMSA's token. In practice the sMSA's Domain Admins"
    Write-HelpDetail "group membership is not reliably resolved in that token context, so the"
    Write-HelpDetail "sMSA effectively has no access to paths locked down to Domain Admins only."
    Write-HelpDetail "Explicit ACEs bypass group token resolution and are always evaluated."
    Write-Host ""
    Write-HelpLine "Install directory -- ReadAndExecute (ContainerInherit + ObjectInherit):"
    Write-HelpDetail "Grants the sMSA read and execute access to every file and subfolder"
    Write-HelpDetail "inside the install directory, including the Modules\ subfolder."
    Write-HelpDetail "Without this ACE, Get-ChildItem on Modules\ returns 0 results when the"
    Write-HelpDetail "task runs -- no modules load, no jobs are registered, nothing executes."
    Write-Host ""
    Write-HelpLine "Reports folder -- Modify (ObjectInherit):"
    Write-HelpDetail "Grants the sMSA write access to the Reports\ folder so it can create"
    Write-HelpDetail "the unattended log, write audit CSV files, and update the debug log."
    Write-HelpDetail "Without this ACE, the unattended log is never created and all file"
    Write-HelpDetail "output from scheduled runs silently fails."
    Write-Host ""
    Write-HelpModify "Install directory ACL  (explicit ReadAndExecute ACE for sMSA)"
    Write-HelpModify "Reports folder ACL     (explicit Modify ACE for sMSA)"
    Write-HelpNote "This step runs after Step 3 (sMSA creation) so the account exists in AD"
    Write-HelpNote "before its SID is written into the ACLs."

    # =========================================================================
    Write-HelpHeader "Post-Installation"

    Write-HelpSection "Install DSInternals PowerShell Module"
    ...
    # =========================================================================
    Write-HelpSection "Installation Verification"
    Write-HelpLine "After all steps complete, Test-ADPowerAdminInstall runs automatically"
    Write-HelpLine "and verifies the following ten checks:"
    Write-Host ""
    Write-HelpDetail "[CHECK 1]   sMSA account exists in Active Directory"
    Write-HelpDetail "[CHECK 2]   sMSA is a member of Domain Admins"
    Write-HelpDetail "[CHECK 3]   Default Domain Controllers Policy has sMSA SeServiceLogonRight"
    Write-HelpDetail "[CHECK 4]   Scheduled task 'AD-PowerAdmin_Daily' exists and is enabled"
    Write-HelpDetail "[CHECK 5]   System File System audit policy is enabled (Success + Failure)"
    Write-HelpDetail "[CHECK 6]   Install directory has the correct SACL audit entries"
    Write-HelpDetail "[CHECK 7]   DSInternals module is installed"
    Write-HelpDetail "[CHECK 8]   sMSA has explicit ReadData access to the install directory"
    Write-HelpDetail "[CHECK 9]   Reports folder exists at `$global:ReportsPath"
    Write-HelpDetail "[CHECK 10]  sMSA has explicit WriteData access to the Reports folder"
    Write-Host ""
    Write-HelpLine "If any check fails, re-run the installer. The installer is idempotent"
    Write-HelpLine "and will repair missing or incorrect components without duplicating"
    Write-HelpLine "items that are already correct."
    Write-Host ""
    Write-HelpLine "When all checks pass, relaunch from the install directory:"
    ...
    # #########################################################################
    Write-HelpHeader "Part 4: Troubleshooting"
    Write-Host ""
    Write-HelpLine "The most common scheduled task failures trace back to one root cause:"
    Write-HelpLine "the sMSA lacks explicit filesystem permissions on the install directory"
    Write-HelpLine "or the Reports folder. The fix in every case is the same: re-run"
    Write-HelpLine "'Install AD-PowerAdmin' from the installer submenu. Step 6 of the"
    Write-HelpLine "installer always repairs both ACLs unconditionally."

    # =========================================================================
    Write-HelpSection "Symptom: Unattended Log Not Created"
    Write-HelpLine "File expected: `$global:ReportsPath\AD-PowerAdmin_Unattended.log"
    Write-HelpLine "Observed: file is missing or empty after the scheduled task runs."
    Write-Host ""
    Write-HelpLine "Root cause:"
    Write-HelpDetail "The sMSA's token does not resolve Domain Admins group membership when"
    Write-HelpDetail "Windows evaluates filesystem ACLs in a scheduled task context. The"
    Write-HelpDetail "Reports folder is locked to Domain Admins only, so the sMSA has no"
    Write-HelpDetail "write access. PowerShell silently fails to start the log transcript."
    Write-Host ""
    Write-HelpLine "Fix: Re-run the installer."
    Write-HelpDetail "Step 6 calls Set-ReportsFolderAcl, which writes an explicit Modify ACE"
    Write-HelpDetail "for the sMSA on the Reports folder. Explicit ACEs are always evaluated"
    Write-HelpDetail "regardless of group token resolution in the task context."
    Write-Host ""
    Write-HelpLine "How to confirm the fix worked:"
    Write-HelpDetail "Run 'Diagnose Scheduled Task' from the installer submenu. It triggers"
    Write-HelpDetail "the task, waits for it to finish, then checks whether the unattended"
    Write-HelpDetail "log was created and shows its last 10 lines."

    # =========================================================================
    Write-HelpSection "Symptom: Task Runs but No Jobs Execute"
    Write-HelpLine "Observed in unattended log: 'Registered jobs : 0' or '[WARN] No jobs"
    Write-HelpLine "are registered' immediately after the run-start header. No audits run,"
    Write-HelpLine "no emails are sent, and the task exits in seconds."
    Write-Host ""
    Write-HelpLine "Root cause:"
    Write-HelpDetail "AD-PowerAdmin loads modules by calling Get-ChildItem on the Modules\"
    Write-HelpDetail "subfolder inside the install directory. If the sMSA has no read access"
    Write-HelpDetail "to the install directory, Get-ChildItem returns 0 files, no modules"
    Write-HelpDetail "are imported, and the global job table stays empty. The script then"
    Write-HelpDetail "exits immediately with a warning that no jobs are registered."
    Write-Host ""
    Write-HelpLine "Fix: Re-run the installer."
    Write-HelpDetail "Step 6 writes an explicit ReadAndExecute ACE for the sMSA on the install"
    Write-HelpDetail "directory with ContainerInherit + ObjectInherit flags. This propagates"
    Write-HelpDetail "to the Modules\ subfolder and all module files automatically."
    Write-Host ""
    Write-HelpLine "How to confirm the fix worked:"
    Write-HelpDetail "Run 'Diagnose Scheduled Task'. The unattended log should now show"
    Write-HelpDetail "[OK] for each module and a non-zero 'Registered jobs' count."

    # =========================================================================
    Write-HelpSection "Symptom: Stop-Transcript Warnings in the Event Log"
    Write-HelpLine "Observed: Windows PowerShell log entries from the sMSA account with"
    Write-HelpLine "'InvalidOperation,Stop-TranscriptCommand' errors."
    Write-Host ""
    Write-HelpLine "Root cause:"
    Write-HelpDetail "PowerShell 5.1 only supports one active transcript at a time. If a"
    Write-HelpDetail "previous session's transcript was not cleanly stopped, the next run"
    Write-HelpDetail "encounters a conflict on Stop-Transcript. These warnings are logged"
    Write-HelpDetail "but are handled internally by Stop-AllTranscripts and do not prevent"
    Write-HelpDetail "the script from running."
    Write-Host ""
    Write-HelpLine "Action required: None in isolation."
    Write-HelpDetail "These warnings are cosmetic when they appear alone. If they appear"
    Write-HelpDetail "together with missing logs or zero registered jobs, treat the other"
    Write-HelpDetail "symptom as the root cause and re-run the installer."

    # =========================================================================
    Write-HelpSection "Diagnostic Tool: Diagnose Scheduled Task"
    Write-HelpLine "Access: AD-PowerAdmin Management submenu --> select"
    Write-HelpLine "'Diagnose Scheduled Task' from the menu list."
    Write-Host ""
    Write-HelpLine "What it does:"
    Write-HelpDetail "1. Triggers the AD-PowerAdmin_Daily scheduled task immediately."
    Write-HelpDetail "2. Polls Task Scheduler until the task reports 'Ready' (run complete)."
    Write-HelpDetail "3. Searches the Task Scheduler Operational log for task start/stop"
    Write-HelpDetail "   events scoped to the window of this specific run."
    Write-HelpDetail "4. Searches the PowerShell Operational log for warnings and errors"
    Write-HelpDetail "   generated during the run (levels 1-3 only)."
    Write-HelpDetail "5. Checks the classic Windows PowerShell log for error events."
    Write-HelpDetail "6. Shows the last 10 lines of the unattended log."
    Write-HelpDetail "7. Exports all collected events to a CSV in the Reports folder."
    Write-Host ""
    Write-HelpLine "Key things to look for in the output:"
    Write-HelpDetail "Unattended log missing or empty   --> sMSA write permission issue (Step 6)"
    Write-HelpDetail "'Registered jobs : 0'             --> sMSA read permission issue (Step 6)"
    Write-HelpDetail "[OK] lines for each .psd1 file    --> modules loading correctly"
    Write-HelpDetail "Error events in PowerShell logs   --> module or script errors to fix"
    Write-Host ""
    Write-HelpNote "The tool exports events to a CSV in Reports\ so you can review more"
    Write-HelpNote "than the 15-event console cap. Check the Reports folder for the file."

    # =========================================================================
    Write-HelpSection "Reading Module Load Status in the Unattended Log"
    Write-HelpLine "When AD-PowerAdmin starts in unattended mode it logs the result of"
    Write-HelpLine "loading each module before dispatching any jobs."
    Write-Host ""
    Write-HelpLine "Status codes written to the log for each .psd1 file:"
    Write-HelpDetail "[OK]   <module.psd1>  -- module imported and menu entries registered"
    Write-HelpDetail "[SKIP] <module.psd1>  -- module requires a higher PowerShell version"
    Write-HelpDetail "[FAIL] <module.psd1>  -- import failed; the error message follows"
    Write-Host ""
    Write-HelpLine "If the log shows '.psd1 files found : 0':"
    Write-HelpDetail "The sMSA has no read access to the Modules\ folder. Re-run the"
    Write-HelpDetail "installer. Step 6 adds the explicit ReadAndExecute ACE that fixes this."
    Write-Host ""
    Write-HelpNote "Module load status appears near the top of the unattended log, before"
    Write-HelpNote "the '=== Unattended Run Start' line."
