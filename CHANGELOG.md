...
**Added:**
- Mission statement section inserted between the Overview and Installation sections. Explains the
  origin of the project (personal automation toolset grown from operational need), the
  penetration-testing methodology that drives every feature (learn the technique, build the
  defense, automate the enforcement), the password-security focus and the reasoning behind the
  HIBP and DSInternals integration, and the auditability goal of producing a structured,
  schedulable record of security checks. No code changes; documentation only.

---

### [AD-PowerAdmin_Mgr — Randomized Account Password]

**Added:**
- `Set-AdAccountRandomPassword` -- searches for an AD user account (enabled or disabled),
  displays account details, and requires the administrator to type YES before applying a
  cryptographically random 64-character password via `Set-ADAccountPassword -Reset`. The
  password is generated by `New-RandomPassword -Length 64 -AsSecureString` and discarded
  immediately after being set; it is never displayed or stored.

  Motivation: distribution-list mailbox accounts, legacy service accounts, and other user
  objects that exist only for AD functionality (not interactive login) often retain their
  original passwords indefinitely after they go out of active use. Setting a strong unknown
  password on such accounts eliminates that residual credential exposure without decommissioning
  the account or disrupting services that depend on the AD object.

  The search intentionally includes disabled accounts so that already-disabled objects can
  be targeted without re-enabling them first. Registered as a submenu item under the
  existing AD Management (MgrMenu) sub-menu.
